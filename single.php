...
if(isset($_GET['id']) & !empty($_GET['id'])){
	...
	//$prodnum = mysqli_num_rows($prodres);
	//echo $prodnum;
								...
								<div class="col-md-7 product-single">
									...
									<div class="product-meta">
										<span>Categories:
											...
											?>
											<a href="#"><?php echo $prodcatr['name'];?></a><br>
										</div>
									</div>
								</div>
...
<div style="" class="tab-pane fade" id="mini-three">
	<div class="col-md-12">
		<?php $revcountsql = "SELECT count(*) AS count FROM reviews r
		WHERE r.pid=$id";
		...
		<ul class="comment-list">
			...
			FROM reviews r JOIN usersmeta u WHERE r.uid=u.uid AND r.pid=$id";
...
<?php } ?>
		...
		if ($chkrevr['reviewcount']==1) {
			echo "<h4 class='uppercase space20'>You have already submitted a review</h4>";
		}
		else {
		...
		<form id="form" class="review-form" method="POST">
			<?php
			$usersql = "SELECT u.email , u1.firstname , u1.lastname FROM users u
			JOIN usersmeta u1 WHERE u.id=u1.uid AND u.id='$uid'";
